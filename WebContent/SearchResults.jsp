...
<c:choose>
	...
	<p>There were no results matching your query</p>
	</c:when>
	<c:otherwise>
	<h1>
	...
	<table id ="list">
		...
		<td><a href="dispatcher?operation=entryinfo&i=${i}"><c:out value="${results[i].title}"/></a></td>
		...
		<c:choose>
			...
			${message}
			 ...
			 <c:otherwise>
			 <form action = "dispatcher?operation=entryinfo&i=${i}&from=home" method = "POST">
			 <input type = "submit" name = "action" value ="Add to cart"></form>
			 </c:otherwise>
		</c:choose>
		</td>
		</tr>
